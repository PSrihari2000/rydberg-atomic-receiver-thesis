...
% GD_ESTIMATOR_1D  Exact minimizer of the GD objective, Eq.(13)-(15). Copied
% unchanged from fig3/ for Fig.4's "GD" (unconstrained, no rank projection)
% curve: with no rank constraint, Eq.21's 2D objective decomposes into I1*I2
% independent per-antenna-pair K-unknown LS problems, IDENTICAL in structure
% to the 1D per-antenna decomposition this function was originally derived
% for -- so it is called here on the (I1*I2) x P flattened 2D data as-is,
% with Y/absB/Z's rows now indexing antenna PAIRS instead of a 1D antenna.
%
