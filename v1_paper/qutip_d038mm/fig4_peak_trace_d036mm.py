# ============================================================
# FIG. 4 -- PEAK TRACE DIRECTLY FROM DATA -- d = 0.36 mm
#
# Same honest methodology as fig4_fresh_build/fig4_peak_trace_from_data.py:
# re-runs find_peaks() on every row of the REAL d=0.36mm Pout_surface,
# with NO resolvability threshold and NO separation/interior-valley
# gate. Whatever peak count and position the data actually has at
# each Omega_RF is what gets plotted. The 2xGamma_EIT Rayleigh
# threshold is kept only as a labeled REFERENCE band, exactly as
# before -- it does not gate what the red curve shows, and it is
# NOT presented as "the" saturation point.
# ============================================================
...
print("=" * 70)
print(f"FIG. 4 -- PEAK TRACE DIRECTLY FROM DATA -- d = {d_probe*1e3:.2f} mm")
print("=" * 70)
...
print(f"\n(Reference only, NOT a gate) Gamma_EIT = {Gamma_EIT:.6f} MHz, "
...
# ------------------------------------------------------------
# Cross-check against the d=0.76mm raw peak trace, if present
# ------------------------------------------------------------

ref_path = OUTPUT_DIR.parent / "fig4_fresh_build" / "fig4_peak_trace.npz"
if ref_path.exists():
    ref = np.load(ref_path)
    max_left_diff = np.nanmax(np.abs(left_x - ref["left_x"]))
    max_right_diff = np.nanmax(np.abs(right_x - ref["right_x"]))
    same_npeaks = np.array_equal(n_peaks_found, ref["n_peaks_found"])
    print(f"\nCross-check vs {ref_path.name} (d=0.76mm raw peak trace):")
    print(f"  max |left_x(0.36mm) - left_x(0.76mm)| = {max_left_diff:.3e} MHz")
    print(f"  max |right_x(0.36mm) - right_x(0.76mm)| = {max_right_diff:.3e} MHz")
    print(f"  n_peaks_found arrays identical: {same_npeaks}")
else:
    print(f"\n(No d=0.76mm peak-trace file found to cross-check against.)")

np.savez(
    OUTPUT_DIR / "fig4_peak_trace_d036mm.npz",
    omega_rf_mhz=omega_rf_mhz, delta_c_mhz=delta_c_mhz,
    left_x=left_x, right_x=right_x, n_peaks_found=n_peaks_found,
    Gamma_EIT=Gamma_EIT, required_separation_mhz=required_separation_mhz, d_probe=d_probe,
)
print(f"\nSaved: fig4_peak_trace_d036mm.npz")

import csv
with open(OUTPUT_DIR / "fig4_peak_trace_d036mm.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
print(f"Saved: fig4_peak_trace_d036mm.csv")
...
ax.set_title(f"Fig. 4 (data-driven) -- peak positions, no resolvability gate\n(d = {d_probe*1e3:.2f} mm)")
ax.set_xlim(-9, 9)
ax.set_ylim(0, 16)
ax.legend(fontsize=9, loc="upper right")
ax.grid(alpha=0.3)
fig.tight_layout()
fig.savefig(OUTPUT_DIR / "fig4_peak_trace_d036mm.png", dpi=220, bbox_inches="tight")
plt.close(fig)
print("Saved: fig4_peak_trace_d036mm.png")
print("\nDONE.")
